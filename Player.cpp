...
virtual int Player::checkScore()
{
    ...
    int acesCount = std::count(hand.begin(), hand.end(), [](Card &c) {return (c.checkValue()==1);});
    tempScore += std::accumulate(hand.begin(), hand.end(), [](Card &c) {return (c.checkValue()!=1);});
    // if we are above 21 or we already checked all cards then the tempScore will be also the final one and should be returned
    if (tempScore > 21 || acesCount == 0 ) return tempScore;
    // if adding only aces counted as 1 we get 21 or higher, as this would be the minimum score we return the result
    if (tempScore + acesCount >= 21) return tempScore + acesCount;
